...
| `homeConfigurations."hosts/<host>/<user>"` | `lib/mkHomeConfigurations.nix` |
| `systemConfigs.<host>` (also `.hosts.<host>`, `.<system>.<host>`) | `lib/mkSystemManagerConfigurations.nix` |
| `deploy.nodes.<host>` | `lib/mkDeployNodes.nix` — hosts with an `ip` and `kind = "nixos"` |
...
darwin/profiles/                # macOS base (pulls in nixos/modules/nix.nix, users, home-manager)
...
```bash
just switch <host>          # nixos-rebuild
just darwin <host>          # darwin-rebuild
