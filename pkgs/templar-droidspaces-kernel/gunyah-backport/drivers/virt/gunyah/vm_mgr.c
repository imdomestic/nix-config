...
/* rb_find() and rb_find_add() were added after Android's 5.10 baseline. */
static struct rb_node *gh_rb_find(const void *key, const struct rb_root *tree,
...
static struct rb_node *gh_rb_find_add(struct rb_node *node,
		struct rb_root *tree,
								...
								u64 len, u64 data)
{
	...
	node = gh_rb_find(&key, &ghvm->mmio_handler_root, gh_vm_io_handler_find);
	if (!node)
		return NULL;
...
int gh_vm_add_io_handler(struct gh_vm *ghvm, struct gh_vm_io_handler *io_hdlr)
{
	...
	found = gh_rb_find_add(&io_hdlr->node, &ghvm->mmio_handler_root,
			       gh_vm_io_handler_compare);
...
{
	struct ghh_rm_vm_status_payload *payload = data;
...
{
	struct ghh_rm_vm_exited_payload *payload = data;
...
static void gh_vm_stop(struct gh_vm *ghvm)
{
	...
	if (ghvm->vm_status == GH_RM_VM_STATUS_RUNNING) {
		ret = ghh_rm_vm_stop(ghvm->rm, ghvm->vmid);
		if (ret)
...
static int gh_vm_start(struct gh_vm *ghvm)
{
	...
	ret = ghh_rm_notifier_register(ghvm->rm, &ghvm->nb);
	if (ret)
		goto err;

	ret = ghh_rm_alloc_vmid(ghvm->rm, 0);
	if (ret < 0) {
		ghh_rm_notifier_unregister(ghvm->rm, &ghvm->nb);
		goto err;
	}
	ghvm->vmid = ret;
	...
	list_for_each_entry(mapping, &ghvm->memory_mappings, list) {
		...
		case VM_MEM_LEND:
			ret = ghh_rm_mem_lend(ghvm->rm, &mapping->parcel);
			break;
		case VM_MEM_SHARE:
			ret = ghh_rm_mem_share(ghvm->rm, &mapping->parcel);
			break;
		}
		if (ret) {
	...
	ret = ghh_rm_vm_configure(ghvm->rm, ghvm->vmid, ghvm->auth, mem_handle,
	...
	if (ghvm->auth == GH_RM_VM_AUTH_QCOM_ANDROID_PVM) {
		...
		ret = ghh_rm_vm_set_firmware_mem(ghvm->rm, ghvm->vmid, &mapping->parcel,
	...
	ret = ghh_rm_vm_init(ghvm->rm, ghvm->vmid);
	if (ret) {
	...
	ret = ghh_rm_get_hyp_resources(ghvm->rm, ghvm->vmid, &resources);
	if (ret) {
	...
	for (i = 0, n = le32_to_cpu(resources->n_entries); i < n; i++) {
		ghrsc = ghh_rm_alloc_resource(ghvm->rm, &resources->entries[i]);
		if (!ghrsc) {
			ret = -ENOMEM;
			goto err;
		}
	...
	ret = ghh_rm_vm_start(ghvm->rm, ghvm->vmid);
	if (ret) {
...
static long gh_vm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GH_VM_SET_DTB_CONFIG: {
		...
		if (dtb_config.guest_phys_addr > U64_MAX - dtb_config.size)
			return -EOVERFLOW;
...
static void gh_vm_free(struct work_struct *work)
{
	...
	if (ghvm->vm_status != GH_RM_VM_STATUS_NO_STATE &&
		...
		ret = ghh_rm_vm_reset(ghvm->rm, ghvm->vmid);
		if (ret)
	...
	if (ghvm->vm_status > GH_RM_VM_STATUS_NO_STATE) {
		...
		ret = ghh_rm_dealloc_vmid(ghvm->rm, ghvm->vmid);
		if (ret)
