...
static int gh_vcpu_rm_notification(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	struct ghh_rm_vm_exited_payload *exit_payload = data;
...
static inline enum gh_vm_status remap_vm_status(enum ghh_rm_vm_status rm_status)
{
	switch (rm_status) {
...
static long gh_vcpu_bind(struct gh_vm_function_instance *f)
{
	 ...
	 */
	vcpu->nb.priority = -1;
	r = ghh_rm_notifier_register(f->rm, &vcpu->nb);
	if (r)
		goto err_put_gh_vm;
...
err_put_fd:
	put_unused_fd(fd);
err_notifier:
	ghh_rm_notifier_unregister(f->rm, &vcpu->nb);
err_put_gh_vm:
	gh_vm_put(vcpu->ghvm);
