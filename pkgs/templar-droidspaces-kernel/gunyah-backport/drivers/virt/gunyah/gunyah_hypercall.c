...
{
	register unsigned long x0 asm("x0") = GH_HYPERCALL_VCPU_RUN;
	register unsigned long x1 asm("x1") = capid;
	register unsigned long x2 asm("x2") = resume_data[0];
	register unsigned long x3 asm("x3") = resume_data[1];
	register unsigned long x4 asm("x4") = resume_data[2];
	register unsigned long x5 asm("x5") = 0;

	/* Android 5.10 only exposes the four-result SMCCC helper. VCPU_RUN
	 * returns state_data[2] in x4, so preserve the 6.1 register ABI with a
	 * local six-register HVC sequence.
	 */
	asm volatile("hvc #0"
	...
	if (x0 == GH_ERROR_OK) {
		resp->sized_state = x1;
		resp->state_data[0] = x2;
		resp->state_data[1] = x3;
		resp->state_data[2] = x4;
	}
