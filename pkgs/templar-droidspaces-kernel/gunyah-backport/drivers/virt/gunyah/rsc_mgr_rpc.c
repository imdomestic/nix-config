...
struct ghh_rm_vm_common_vmid_req {
	__le16 vmid;
	__le16 _padding;
} __packed;
...
struct ghh_rm_mem_share_req_header {
	u8 mem_type;
	u8 _padding0;
	u8 flags;
	u8 _padding1;
...
struct ghh_rm_mem_share_req_acl_section {
	__le32 n_entries;
	struct ghh_rm_mem_acl_entry entries[];
};

struct ghh_rm_mem_share_req_mem_section {
	__le16 n_entries;
	__le16 _padding;
	struct ghh_rm_mem_entry entries[];
};

/* Call: MEM_RELEASE */
struct ghh_rm_mem_release_req {
	__le32 mem_handle;
...
struct ghh_rm_mem_append_req_header {
	__le32 mem_handle;
	u8 flags;
	u8 _padding0;
	__le16 _padding1;
...
/* Call: VM_ALLOC */
struct ghh_rm_vm_alloc_vmid_resp {
	__le16 vmid;
	__le16 _padding;
} __packed;
...
struct ghh_rm_vm_config_image_req {
	__le16 vmid;
	__le16 auth_mech;
	__le32 mem_handle;
	__le64 image_offset;
 ...
 */
static int ghh_rm_common_vmid_call(struct gh_rm *rm, u32 message_id, u16 vmid)
{
	struct ghh_rm_vm_common_vmid_req req_payload = {
		.vmid = cpu_to_le16(vmid),
	};

	return ghh_rm_call(rm, message_id, &req_payload, sizeof(req_payload), NULL, NULL);
}

static int _gh_rm_mem_append(struct gh_rm *rm, u32 mem_handle, bool end_append,
			struct ghh_rm_mem_entry *mem_entries, size_t n_mem_entries)
{
	struct ghh_rm_mem_share_req_mem_section *mem_section;
	struct ghh_rm_mem_append_req_header *req_header;
	size_t msg_size = 0;
	void *msg;
	int ret;

	msg_size += sizeof(struct ghh_rm_mem_append_req_header);
	...
	ret = ghh_rm_call(rm, GH_RM_RPC_MEM_APPEND, msg, msg_size, NULL, NULL);
	kfree(msg);

	return ret;
}

static int ghh_rm_mem_append(struct gh_rm *rm, u32 mem_handle,
			struct ghh_rm_mem_entry *mem_entries, size_t n_mem_entries)
{
	bool end_append;
	int ret = 0;
...
static int ghh_rm_mem_lend_common(struct gh_rm *rm, u32 message_id, struct ghh_rm_mem_parcel *p)
{
	size_t msg_size = 0, initial_mem_entries = p->n_mem_entries, resp_size;
	size_t acl_section_size, mem_section_size;
	struct ghh_rm_mem_share_req_acl_section *acl_section;
	struct ghh_rm_mem_share_req_mem_section *mem_section;
	struct ghh_rm_mem_share_req_header *req_header;
	u32 *attr_section;
	__le32 *resp;
	void *msg;
	int ret;
	 ...
	 */
	msg_size += sizeof(struct ghh_rm_mem_share_req_header);
	...
	ret = ghh_rm_platform_pre_mem_share(rm, p);
	if (ret) {
		kfree(msg);
		return ret;
	}
	...
	*attr_section = 0;
	...
	ret = ghh_rm_call(rm, message_id, msg, msg_size, (void **)&resp, &resp_size);
	kfree(msg);

	if (ret) {
		ghh_rm_platform_post_mem_reclaim(rm, p);
		return ret;
	}
...
/**
 * ghh_rm_mem_lend() - Lend memory to other virtual machines.
 ...
 */
int ghh_rm_mem_lend(struct gh_rm *rm, struct ghh_rm_mem_parcel *parcel)
{
	return ghh_rm_mem_lend_common(rm, GH_RM_RPC_MEM_LEND, parcel);
}


/**
 * ghh_rm_mem_share() - Share memory with other virtual machines.
 * @rm: Handle to a Gunyah resource manager
 ...
 */
int ghh_rm_mem_share(struct gh_rm *rm, struct ghh_rm_mem_parcel *parcel)
{
	return ghh_rm_mem_lend_common(rm, GH_RM_RPC_MEM_SHARE, parcel);
}

/**
 * ghh_rm_mem_reclaim() - Reclaim a memory parcel
 * @rm: Handle to a Gunyah resource manager
 ...
 */
int ghh_rm_mem_reclaim(struct gh_rm *rm, struct ghh_rm_mem_parcel *parcel)
{
	struct ghh_rm_mem_release_req req = {
		.mem_handle = cpu_to_le32(parcel->mem_handle),
	};
	int ret;

	ret = ghh_rm_call(rm, GH_RM_RPC_MEM_RECLAIM, &req, sizeof(req), NULL, NULL);
	/* Only call the platform mem reclaim hooks if we reclaimed the memory */
	if (ret)
		return ret;

	return ghh_rm_platform_post_mem_reclaim(rm, parcel);
}

/**
 * ghh_rm_vm_set_firmware_mem() - Set the location of firmware for GH_RM_VM_AUTH_QCOM_ANDROID_PVM VMs
 * @rm: Handle to a Gunyah resource manager.
 * @vmid: VM identifier allocated with ghh_rm_alloc_vmid.
 ...
 */
int ghh_rm_vm_set_firmware_mem(struct gh_rm *rm, u16 vmid, struct ghh_rm_mem_parcel *parcel,
				u64 fw_offset, u64 fw_size)
{
	...
	return ghh_rm_call(rm, GH_RM_RPC_VM_SET_FIRMWARE_MEM, &req, sizeof(req), NULL, NULL);
}
EXPORT_SYMBOL_GPL(ghh_rm_vm_set_firmware_mem);

/**
 * ghh_rm_alloc_vmid() - Allocate a new VM in Gunyah. Returns the VM identifier.
 * @rm: Handle to a Gunyah resource manager
 ...
 */
int ghh_rm_alloc_vmid(struct gh_rm *rm, u16 vmid)
{
	struct ghh_rm_vm_common_vmid_req req_payload = {
		.vmid = cpu_to_le16(vmid),
	};
	struct ghh_rm_vm_alloc_vmid_resp *resp_payload;
	size_t resp_size;
	void *resp;
	int ret;

	ret = ghh_rm_call(rm, GH_RM_RPC_VM_ALLOC_VMID, &req_payload, sizeof(req_payload), &resp,
			&resp_size);
	if (ret)
		return ret;
...
/**
 * ghh_rm_dealloc_vmid() - Dispose of a VMID
 * @rm: Handle to a Gunyah resource manager
 * @vmid: VM identifier allocated with ghh_rm_alloc_vmid
 */
int ghh_rm_dealloc_vmid(struct gh_rm *rm, u16 vmid)
{
	return ghh_rm_common_vmid_call(rm, GH_RM_RPC_VM_DEALLOC_VMID, vmid);
}

/**
 * ghh_rm_vm_reset() - Reset a VM's resources
 * @rm: Handle to a Gunyah resource manager
 * @vmid: VM identifier allocated with ghh_rm_alloc_vmid
 *
 ...
 */
int ghh_rm_vm_reset(struct gh_rm *rm, u16 vmid)
{
	return ghh_rm_common_vmid_call(rm, GH_RM_RPC_VM_RESET, vmid);
}

/**
 * ghh_rm_vm_start() - Move a VM into "ready to run" state
 * @rm: Handle to a Gunyah resource manager
 * @vmid: VM identifier allocated with ghh_rm_alloc_vmid
 *
 ...
 * scheduling policies.
 */
int ghh_rm_vm_start(struct gh_rm *rm, u16 vmid)
{
	return ghh_rm_common_vmid_call(rm, GH_RM_RPC_VM_START, vmid);
}

/**
 * ghh_rm_vm_stop() - Send a request to Resource Manager VM to forcibly stop a VM.
 * @rm: Handle to a Gunyah resource manager
 * @vmid: VM identifier allocated with ghh_rm_alloc_vmid
 */
int ghh_rm_vm_stop(struct gh_rm *rm, u16 vmid)
{
	struct ghh_rm_vm_stop_req req_payload = {
		.vmid = cpu_to_le16(vmid),
		.flags = GH_RM_VM_STOP_FLAG_FORCE_STOP,
		.stop_reason = cpu_to_le32(GH_RM_VM_STOP_REASON_FORCE_STOP),
	};

	return ghh_rm_call(rm, GH_RM_RPC_VM_STOP, &req_payload, sizeof(req_payload), NULL, NULL);
}

/**
 * ghh_rm_vm_configure() - Prepare a VM to start and provide the common
 ...
 */
int ghh_rm_vm_configure(struct gh_rm *rm, u16 vmid, enum ghh_rm_vm_auth_mechanism auth_mechanism,
		u32 mem_handle, u64 image_offset, u64 image_size, u64 dtb_offset, u64 dtb_size)
{
	struct ghh_rm_vm_config_image_req req_payload = {
		.vmid = cpu_to_le16(vmid),
	...
	return ghh_rm_call(rm, GH_RM_RPC_VM_CONFIG_IMAGE, &req_payload, sizeof(req_payload),
			  NULL, NULL);
}

/**
 * ghh_rm_vm_init() - Move the VM to initialized state.
 * @rm: Handle to a Gunyah resource manager
 * @vmid: VM identifier
 *
 * RM will allocate needed resources for the VM.
 */
int ghh_rm_vm_init(struct gh_rm *rm, u16 vmid)
{
	return ghh_rm_common_vmid_call(rm, GH_RM_RPC_VM_INIT, vmid);
}

/**
 * ghh_rm_get_hyp_resources() - Retrieve hypervisor resources (capabilities) associated with a VM
 * @rm: Handle to a Gunyah resource manager
 * @vmid: VMID of the other VM to get the resources of
 * @resources: Set by ghh_rm_get_hyp_resources and contains the returned hypervisor resources.
 *             Caller must free the resources pointer if successful.
 */
int ghh_rm_get_hyp_resources(struct gh_rm *rm, u16 vmid,
				struct ghh_rm_hyp_resources **resources)
{
	struct ghh_rm_vm_common_vmid_req req_payload = {
		.vmid = cpu_to_le16(vmid),
	};
	struct ghh_rm_hyp_resources *resp;
	size_t resp_size;
	int ret;

	ret = ghh_rm_call(rm, GH_RM_RPC_VM_GET_HYP_RESOURCES,
...
/**
 * ghh_rm_get_vmid() - Retrieve VMID of this virtual machine
 * @rm: Handle to a Gunyah resource manager
 * @vmid: Filled with the VMID of this VM
 */
int ghh_rm_get_vmid(struct gh_rm *rm, u16 *vmid)
{
	...
	ret = ghh_rm_platform_get_vmid(rm, &cached_vmid);
	if (ret)
		return ret;
...
EXPORT_SYMBOL_GPL(ghh_rm_get_vmid);
