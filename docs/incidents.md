...
保持 QUASAR NVFP4 QAT 权重、MTP3、262144 共享 KV/context、48K 视觉总预算、
16K 单媒体预算与 Host-KV 配置不变。客户端名称仍为 `qwen3.8-27b`。

测试脚本 `scripts/bench-qwen38-concurrency.py`：暂时停止 llama-swap 及对外
网关，重启后端清空缓存，经相同 SSH 隧道直连 8100；先 warmup，再分别跑
数数与混合生成（Python 代码、中文小说、JSON 书目）。每类两轮，每轮同时
提交 12 个请求，由 C3/C4 调度，输出上限均为 2048 tokens，关闭 thinking。
每个请求用不同前缀避免缓存，两配置的 48 个 payload SHA256 一一相同；
实际缓存命中全部为零。数数 temperature=0，混合=1，seed 同步。

| 负载 | C3 整批平均 tok/s | C4 整批平均 tok/s | 增幅 |
|---|---:|---:|---:|
| 数数 | 658.49 | 847.78 | 28.75% |
| 混合生成 | 403.77 | 502.69 | 24.50% |

整批平均是两轮实际 completion tokens 之和除以两轮 makespan 之和，包含
排队、prefill 和尾部收尾，不取服务瞬时峰值。C3/C4 数数每轮均输出 24576
个 tokens，耗时分别 37.31/37.33 s 与 28.97/29.00 s。混合生成 C3 两轮为
411.90/396.30 tok/s，C4 为 467.62/543.46 tok/s；有明显轮间波动。
C3 混合第一轮一条请求提前 EOS，因此该轮实际 23467 tokens，其余三轮
均为 24576，不能拿固定上限冒充实际输出。含排队的请求耗时中位数：
数数 23.33→19.36 s，混合 34.99→32.81 s。

日志按实例与每轮时间边界核对，C3 为 `serve-1-1790317330219479`，C4 为
`serve-1-1790317593223495`；实际 `running`/`decode_ready` 达到 3/4。
数数 MTP 接受率 99.86%→99.88%，混合 51.40%→51.62%。活跃请求自身
decode 耗时中位数数数 9.12→9.38 s、混合 11.81→12.16 s：吞吐和排队
改善并不意味着每路解码更快。

误导点是把数数接近 100% 的接受率推广到日常请求，或把逐路速率/瞬时峰值
相加。混合生成也是短 prompt 合成负载，不是 Max 长上下文日志回放；两轮
不足以给出各种真实业务的固定增幅，报告应保留范围及实际输出量。

C3/C4 启动空闲显存 500.625→340.625 MiB，增加一路约用 160 MiB；GPU
采样 C3 最低空闲 501 MiB，C4（含并发多图验收）最低 339 MiB。四路仍共享
262144 KV，不能解读为每路都有独立 262K 容量。

三路各 8192 输出上限的文字生成同时进行时，满 48K 的三图请求通过，实际
prompt 49186 tokens，正确输出 Red/Green/Blue；四图 65536 视觉 tokens
按预期 HTTP 400 `media_budget_exceeded`。该期间日志记录 `running=4`、
`decode_ready=3`、`prefilling=1`。无服务自动重启，无内核 OOM/GPU Xid。
网关恢复后六项文字/SSE/工具/图片回归全部通过，保留 C4；系统闭包构建、
激活及运行 store path 比对通过。原始 JSON、GPU 采样与分实例日志存于 b650
`/var/lib/qwen38/validation/20260925-c3-c4/`。
...
服务及 fast 别名，最终对外只保留 `qwen3.8-27b`。先验收单并发，再按用户要求
升至三并发。NVFP4 权重、NVFP4 KV、MTP3、视觉同时启用，context/KV 为
...
对照阶段 long 使用旧镜像，最终停用其服务；旧镜像及权重文件保留供回退。
旧引擎的 catalog 修复不能等同于新引擎的验收。
...
在 b650 RTX 5090 D v2（24,455 MiB）单并发实测启动成功：权重 17,542,223,360
...
```sh
python3 scripts/test-qwen38-quasar.py \
  --url http://b650.inner.imdomestic.com:8000 \
  --model qwen3.8-27b --output REPORT.json
```
...
短输入补测（独立随机前缀，无缓存命中，均输出 `OK`）：

| 实际 prompt tokens | Prefill | Prefill 耗时 |
|---:|---:|---:|
| 2043 | 11552 tok/s | 0.177 s |
| 8188 | 13842 tok/s | 0.592 s |
| 32768 | 10084 tok/s | 3.250 s |

提升为唯一模型前，在暂停网关、全新后端上运行
`scripts/test-qwen38-kv-cache.py`，17 个请求的回归通过。7 个 45096-token
会话重放均命中 45091 tokens；Main-KV 换出 1,824,915,456 bytes、换入
990,904,320 bytes，Host-KV 峰值 1,936,392,192 bytes。第九个 owner 只淘汰
一个旧 owner，maximal fallback 为 0，最近重用的会话仍能恢复。

三并发实测启动后剩余 524,943,360 bytes（500.6 MiB），`nvidia-smi` 驻留
23476 MiB、可用 501 MiB。三个随机前缀的数数请求同时发送，总输出 4875 tokens，
端到端 8.49 s，总吞吐 574.19 tok/s；各路 API decode 约 223–225 tok/s。
日志确认 running=3、decode-ready=3，包含平均 batch=2.80 的五秒采样窗口。
同一 C3 配置只发一路时 decode 为 239.06 tok/s。数数的 MTP 接受率接近 100%，
不能据此推断复杂代码/推理也有同样速度；同期实际带图和 30 个工具定义的
23695-token 请求，prefill 约 10.7K tok/s、decode 138 tok/s、MTP 接受率 47.6%。
三路共享 262144-token Device-KV 池，不是每路各有一个完整的 262K 池。
升为 C3 后重跑七项验收全部通过；259238-token 文本＋图片的 prefill 为
102.71 s（2524 tok/s）、decode 172.53 tok/s。该请求排队约 10.3 s，运行时
另一个请求因共享 KV 池不足排队，完成后继续处理。实测显存可用最低约
499 MiB，服务无自动重启，未发现 CUDA OOM 或 GPU Xid。

这些合成测试验证功能和容量，不是通用模型质量或真实文档速度评测，也没有
证明 QUASAR 的精度优于旧量化。报告存于 b650
`/var/lib/qwen38/validation/20260924-quasar/`。网关和 OpenCode 仅保留
`qwen3.8-27b`，context 为 262144，OpenCode output 同为 262144。
...
`get_status` 同时返回 `online=false, good=true`，WebUI 报告登录身份失效。
告警模块只检查适配器连接，仍每约 30 秒发送一次，收到 1006514 网络异常。
这些错误不能反推主机断网，更不能把 `good=true` 当成 QQ 已登录。

修复在发送告警前验证真实账号状态，失败后指数退避，上限 5 分钟；保留待处理
告警和历史采集，不把离线跳过标成已发送。独立 timer 检查账号并输出带检查时间
的 Prometheus 指标，控制台通过活动告警显示账号离线、需扫码或检查过期。
连续三次传输故障才尝试恢复，15 分钟冷却、每天最多三次，预算跨服务重启保留；
登录失效、管理端认证失败和人工停止都不触发自动重启。

NapCat 使用官方 v4.18.28 固定 amd64 镜像，保持原 QQ/配置目录。此升级不代表
腾讯登录凭证失效已获根治：上游没有提供永久在线保证，撤销的凭证仍需用户扫码。
先前容易误判的证据是 WebSocket 重连和 systemd active，它们只证明程序间连接。
...
用户要求继续清理后，删除未使用的 Codex 0.144.6/0.146.0 程序包（保留当前
0.147.0 和上一版 0.146.1）、两份 uv 下载缓存、五份旧部署源码副本。核对当前
及仍被停止任务引用的两版镜像后，只移除已删除 Kennethbot 镜像留下的四个
沙盒 GC 根；其余依赖仍被引用，这一步实际仅释放 900.3 KiB，不能按旧基础包
体积虚报。新旧沙盒挂载只读缓存后，Python、Node 和 Nix 均能启动。
...
实际配置为 `ZSH_AUTOSUGGEST_STRATEGY=(history completion)`：历史匹配失败后，
插件还会从普通补全候选中挑选一项。zsh 的 `_command_names` 也提供变量赋值候选，
因此特殊参数 `_` 可以补成 `_=`。

独立 PTY 加载现有配置并清空测试会话历史后，输入 `_` 会出现灰色 `=`，仅有
空格或换行时还会建议 `!`。第一版只保留 `history`，但这会连带关闭所有历史之外
的命令、参数和路径自动预测，范围过大，已撤回。

最终保留历史优先和原有 completion 引擎，用自定义策略包一层：只有补全生成的
完整候选恰为 `_=`（允许前导空白）时才丢弃，其余结果原样返回。Home Manager
的策略选项只接受内建名字，因此函数用 `siteFunctions` 声明，自定义策略名通过
`initContent` 设置；不改全局 completion 样式，也不影响 Tab 候选。

独立 PTY 对比确认：命令、Git 参数、文件路径、历史命令及其他变量赋值的灰色
建议均与原配置一致，只有 `_`、` _`、换行后 `_` 对应的 `=` 被过滤；实际按 Tab
也正常补全测试文件路径。
...
页面验收还发现尾斜杠规范化跳转降到了 HTTP。按路由源地址增加 Docker 桥地址的
信任仍未解决：`getsockname` 显示 `172.17.0.1`，但后端 socket 看到的 peer 被本机
NAT 改成 `10.0.1.1`。绑定 loopback 的同一请求则保留 `127.0.0.1`，并正确返回
HTTPS 跳转。因此 nginx 上游显式 `proxy_bind 127.0.0.1`，应用也只信任 loopback，
不扩大到可能聚合其他来源的 NAT 地址。协议跳转和登录限流据此识别真实代理信息。
...
放行 9100 后,node target 恢复,同一验收又发现 ping exporter 的 9427 仍被拦截;
两个 exporter 都需要遵循同一条边界规则。
...
pane(也就是 `swap-pane -s '{left-of}'` 一族)。写这些绑定时有两处直觉是错的,
把过程和实测记在这里,代码里只留结论。

### swap-pane 不加 -d

最初的错误猜测是照网上配置常见写法 `swap-pane -h -d`:把相邻 pane 换到自己
位置,焦点留在原位。直觉是"我在自己的位子上,不该动",但实际想要的是反过来 ——
交换后焦点跟着**自己**的 pane 走到另一边,体感是"整个人过去了"。
独立 socket 实测(初始 `%0`、`%1` 左右并排):

| 操作 | 结果 |
|---|---|
| 初始 | `%0*@x=0`   `%1@x=41` |
| `swap-pane -s '{right-of}'` | `%1@x=0`   `%0*@x=41` ← 焦点跟着 %0 到了右边 |
| 加 `-d` | `%1@x=0`   `%0@x=41` ← 焦点留在左边,不是想要的 |

### swap-window 必须补 select-window

swap-window 恰好相反:窗口编号互换了,焦点不跟过去:

    0:a 1:b*  --swap-window -t +-->  0:a 1:c* 2:b   ← b 挪走了,人没跟过去

所以绑定写成 `swap-window -t + \; select-window -t +`。

### 到处流传的 `-t -1`

`-t -` / `-t +` 才是 man page 里的 target-window 记号(前一个编号 / 后一个编号),
边界会环绕:从窗口 1 按 `-t -` 落回最后一个窗口,实测正常。博客里流传的
`-t -1` 不是文档写法(看起来像"数组下标减一"),tmux 怎么解析它不可预期,不用。

### 被我带偏的地方

两个错误首猜都来自"照抄知名配置":swap-pane 的 `-d` 是网上配置里的高频出现,
但那是"原位互换、人不动"的语义,不是我要的"交换并跟随";swap-window 的
`-t -1` 是看着像数字的习惯写法,而文档里 target-window 的相对目标就是
`-` / `+`,后面不带数字。
