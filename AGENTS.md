...
- Host metadata: `config.my.host` (declared in `modules/shared/host-options.nix`, injected by the `lib/` builders into every NixOS/darwin/home-manager/system-manager eval) is the single source of truth for `name`, `system`, `roles`, `users`, `usernames`. Modules must read `config.my.host.*` (and home-manager modules `config.home.username` / `config.home.homeDirectory`); the legacy module args (`hostName`, `hostname`, `username`, `usernames`, `hostUsers`, `hostRoles`) were removed. `specialArgs` only carry `inputs`, `system` (needed in `imports`, which cannot read `config`) and `pkgs-unstable`.
